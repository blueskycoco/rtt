...
static int l610_power_on(struct at_device *device)
{
	...
	rt_kprintf("power pin %d\n", l610->power_pin);
    rt_pin_write(L601_W_DIS, PIN_HIGH);
    rt_pin_write(L601_RST_EN, PIN_LOW);
    rt_pin_write(L601_DTR, PIN_HIGH);
    ...
    while (retry_num--)
    {
        ...
        for (i = 0; i < CREG_RETRY; i++)
        {
            ...
            if (!strncmp(parsed_data, "0,1", sizeof(parsed_data)) ||
                !strncmp(parsed_data, "0,3", sizeof(parsed_data)) ||
...
static int l610_init(struct at_device *device)
{
    ...
    if (l610->power_pin != -1 && l610->power_status_pin != -1)
    {
        ...
        rt_pin_mode(L601_DTR, PIN_MODE_OUTPUT);
        rt_pin_mode(L601_W_DIS, PIN_MODE_OUTPUT);
        rt_pin_mode(L601_RST_EN, PIN_MODE_OUTPUT);
