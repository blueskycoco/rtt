...
int sbi_get_cis_info()
{
  ...
  for (i = 0; i < sizeof(tupledata); i++) {
	ret = sdio_read_ioreg(FN0, card->info.cisptr[FN0] + i,
		&tupledata[i]);
	if (ret) {
	  rt_kprintf("get_cis_info error:%d\n", ret);
	  return -1;
	}
  }
...
static int sbi_download_wlan_fw_image(const rt_uint8_t * firmware, int firmwarelen)
{
    rt_uint8_t base0;
    rt_uint8_t base1;
    int ret = 0;
    ...
    int tx_blocks = 0;
    int i;
    rt_kprintf("Downloading FW image (%d bytes)\n", firmwarelen);

    /* Wait initially for the first non-zero value */
    do {
        if ((ret = sdio_read_ioreg(FN1,
                                   HOST_F1_RD_BASE_0, &base0)) < 0) {
            rt_kprintf("Dev BASE0 register read failed:"
                   " base0=0x%04X(%d)\n", base0, base0);
            ret = -1;
            goto done;
        }
        if ((ret = sdio_read_ioreg(FN1,
            ...
            rt_kprintf("Dev BASE1 register read failed:"
                   " base1=0x%04X(%d)\n", base1, base1);
            ret = -1;
            goto done;
        }
        len = (((rt_uint16_t) base1) << 8) | base0;
        rt_thread_delay(1);
    } while (!len && --timeout);

    if (!timeout) {
        rt_kprintf("Helper downloading finished.\n");
        rt_kprintf("Timeout for FW downloading!\n");
        ret = -1;
        goto done;
    }
    ...
    ret = mv_sdio_poll_card_status(CARD_IO_READY | DN_LD_CARD_RDY);
    if (ret < 0) {
        rt_kprintf("FW download died @ the end\n");
        ret = -1;
        goto done;
    }
    ...
    for (offset = 0; offset < firmwarelen; offset += txlen) {
        ...
        /* rt_kprintf(INFO, "fw: offset=%d, txlen = 0x%04X(%d)\n", 
           offset,txlen,txlen); */
        rt_kprintf(".");
        ...
        do {
            ret = sdio_write_iomem(FN1,
                                   card->ioport, BLOCK_MODE,
                                   FIXED_ADDRESS, tx_blocks,
                                   SD_BLOCK_SIZE_FW_DL, fwbuf);

            if (ret < 0) {
                i++;
                rt_kprintf("FW download, write iomem (%d) failed: %d\n", i,
                       ret);
                if (sdio_write_ioreg
                    (FN1, CONFIGURATION_REG, 0x04) < 0) {
                    rt_kprintf("write ioreg failed (FN1 CFG)\n");
                }
                ret = -1;
                if (i > 2) {
                    rt_kprintf("IO error: transferring @ %d\n", offset);
                    goto done;
                }
            }
        } while (ret == -1);
        ...
        if (len & B_BIT_0) {
            rt_kprintf("CRC32 Error indicated by the helper:"
                   " len=0x%04X(%d)\n", len, len);
            len &= ~B_BIT_0;
    ...
    rt_kprintf("\nFW download Succeed, size %d bytes\n", firmwarelen);
...
sbi_prog_firmware_image(helper_sd_bin,helper_sd_bin_size);
sbi_download_wlan_fw_image(sd8686_bin,sd8686_bin_size);
}
