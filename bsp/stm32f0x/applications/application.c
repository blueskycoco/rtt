...
sys_env g_sys_env;
unsigned char hum_interface_state_new=0,hum_interface_state_old=0;/*uninsert */
...
void save_env()
{
  ...
  FLASH_ProgramWord(ENV_FLASH_ADDR+16,(g_sys_env.crc<<16)|g_sys_env.b_hum_local_force_stop);
  FLASH_Lock();
}
int read_env()
{
...
void check_server_command()
{
  ...
  unsigned char command_down[11][13]={
	...
	{	/*got command*/
	  ...
	  {
	  	//wifi_send(buf2,13);
		crc=((int)(~(buf2[1]+buf2[2]+buf2[3]+buf2[4]+buf2[5]+buf2[6]+buf2[7]+buf2[8])+1))%128;
		//buf2[9]=crc;
		//buf2[11]=(g_sys_env.device_id&0xff00)>>8;
		//buf2[12]=g_sys_env.device_id&0xff;
		//wifi_send(buf2,13);
		if(crc>128)
			crc=crc%128;
...
hum_interface_state_old=hum_interface_state_new;
  ...
  while(1)
  {
	...
	{		
		if((x>>8)<g_sys_env.temp_low_val||(x>>8)>g_sys_env.temp_high_val)
		{/*Temp alarm*/
		  buf[index*13+3]=0x07;			
		  buf[index*13+4]=0x11;
		  buf[index*13+7]=0x05;
	...
	/*check hum*/
	if(hum_interface_state_new==0/*GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0)!=Bit_RESET*/)
	{
			...
			else
			{
				...
				else
				{
					...
					if(hum_check_count==g_sys_env.hum_alarm_type)
					{
						...
						count=0;
						while(hum_interface_state_new==0/*GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0)==Bit_SET*/)
						{
							count++;
							if(count>=/*600*/6)
							{
